...
class AudioHandler
{
  public:
    ...
    boolean QueueNotification(unsigned short notificationIndex, unsigned short notificationLength, byte priority, unsigned long currentTime);
    
    boolean Update(unsigned long currentTime);

    boolean StopSound(unsigned short soundIndex);
    boolean StopCurrentNotification();
    boolean StopAllMusic();
    boolean StopAllNotifications();
